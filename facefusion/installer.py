...
def cli() -> None:
	...
	program.add_argument('--onnxruntime', help=wording.get('help.install_dependency').format(dependency='onnxruntime'),
						 choices=ONNXRUNTIMES.keys(), required=True)
...
def run(program: ArgumentParser) -> None:
	...
	subprocess.call([shutil.which('pip'), 'install', '-r', 'requirements.txt', '--force-reinstall', '-i',
	...
	if args.onnxruntime == 'rocm':
		...
		if python_id in ['cp310', 'cp312']:
			...
			subprocess.call(
				[shutil.which('curl'), '--silent', '--location', '--continue-at', '-', '--output', wheel_path,
				 wheel_url])
		...
		subprocess.call(
			[shutil.which('pip'), 'install', onnxruntime_name + '==' + onnxruntime_version, '--force-reinstall', '-i',
			 'https://pypi.tuna.tsinghua.edu.cn/simple'])
	...
	if args.onnxruntime == 'cuda' and has_conda:
		...
		if is_linux():
			...
			library_paths = list(
				dict.fromkeys([library_path for library_path in library_paths if os.path.exists(library_path)]))

			subprocess.call([shutil.which('conda'), 'env', 'config', 'vars', 'set',
		...
		if is_windows():
			...
			subprocess.call(
				[shutil.which('conda'), 'env', 'config', 'vars', 'set', 'PATH=' + os.pathsep.join(library_paths)])

	if args.onnxruntime in ['directml', 'rocm']:
		subprocess.call([shutil.which('pip'), 'install', 'numpy==1.26.4', '--force-reinstall', '-i',
						 'https://pypi.tuna.tsinghua.edu.cn/simple'])
